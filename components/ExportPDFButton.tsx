...
import { exportLessonToPDF, exportProgressReportToPDF, PDFExportLesson, PDFExportProgress } from '@/lib/pdf/pdfExport';
import { logger } from '@/lib/observability/Logger';

interface ExportPDFButtonProps {
  type: 'lesson' | 'progress';
  data: PDFExportLesson | PDFExportProgress;
  className?: string;
}

export default function ExportPDFButton({ type, data, className = '' }: ExportPDFButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      if (type === 'lesson') {
        await exportLessonToPDF(data as PDFExportLesson);
      } else if (type === 'progress') {
        await exportProgressReportToPDF(data as PDFExportProgress);
      }
    } catch (error) {
