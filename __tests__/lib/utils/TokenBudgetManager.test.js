...
const { TokenBudgetManager, GEMINI_PRO_BUDGET } = require('../../../lib/utils/TokenBudgetManager');
...
describe('TokenBudgetManager', () => {
    ...
    describe('checkBudget', () => {
        ...
        test('includes usage percentage', () => {
            const text = 'a'.repeat(1400); // ~400 tokens = 50% of 800
            const result = manager.checkBudget(text);
    ...
    describe('fitWithinBudget', () => {
        ...
        test('truncates fewShot first when over budget', () => {
            const longFewShot = 'a'.repeat(3500); // Way over budget
            const components = {
                system: 'System',
                fewShot: longFewShot,
                session: '',
        ...
        test('never truncates user prompt', () => {
            const userPrompt = 'Important user prompt that must not be truncated';
            const components = {
                system: 'a'.repeat(1000),
                fewShot: 'a'.repeat(1000),
                session: 'a'.repeat(1000),
                rag: 'a'.repeat(1000),
                user: userPrompt
            };

            const result = manager.fitWithinBudget(components);
            // User prompt should remain intact
            expect(result.components.user).toBe(userPrompt);
        });
    });

    describe('getUsageReport', () => {
        test('returns formatted usage bar', () => {
            const text = 'a'.repeat(1400); // ~50%
            const report = manager.getUsageReport(text);
