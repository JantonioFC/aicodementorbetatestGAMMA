...
const db = require('../db'); // Import singleton database instance
...
function migrate() {
    ...
    for (const file of files) {
        // Verificar si ya fue aplicada usando db.get (wrapper)
        const isApplied = db.get('SELECT 1 FROM _migrations WHERE name = ?', [file]);
