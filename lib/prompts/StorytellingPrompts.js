...
const STORYTELLING_LESSON_PROMPT = `${CHAIN_OF_THOUGHT}
 ...
 * @returns {string}
 */
function buildStorytellingPrompt(context, options = {}) {
    let prompt = STORYTELLING_LESSON_PROMPT
    ...
    if (options.studentProfile) {
        prompt = prompt.replace('{student_profile}', options.studentProfile);
    } else {
        prompt = prompt.replace('{student_profile}', '');
    }

    // Agregar contexto RAG si existe
    if (options.ragContext) {
        prompt = `**CONTEXTO DE REFERENCIA:**\n${options.ragContext}\n\n---\n\n${prompt}`;
    }
