...
class TextToSpeechService {
     ...
     * @returns {Promise<Object>} URL o base64 del audio
     */
     ...
     */
    async _synthesizeWithFal(text, voice) {
        try {
     ...
     * Elimina markdown, código, y formatea para mejor pronunciación.
     */
     ...
     */
    _generateSSML(text) {
        // Dividir en párrafos para pausas naturales
        const paragraphs = text.split(/\n\n+/);

        const ssmlParts = paragraphs.map(p =>
            `<p>${p.trim()}</p>`
        );
     ...
     * @param {string} text 
     * @returns {string} JavaScript para ejecutar en el frontend
     */
    generateBrowserScript(text) {
        const cleanText = this._prepareTextForSpeech(text);
...
(function() {
    ...
    // Retornar control del utterance
    return {
        pause: () => synth.pause(),
        resume: () => synth.resume(),
        stop: () => synth.cancel()
     ...
     * @param {number} maxChars - Caracteres máximos por chunk
     * @returns {string[]}
     */
    splitIntoChunks(lessonContent, maxChars = 2000) {
