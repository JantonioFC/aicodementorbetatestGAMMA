...
export async function createReviewRequest(data, userId) {
    ...
    // Security: Validate GitHub URL ONLY if provided (now optional)
    if (data.github_repo_url && !data.github_repo_url.startsWith('https://github.com/')) {
        throw new Error('Invalid GitHub Repository URL. Must start with https://github.com/');
    }

    // Ensure we have at least code or a repo
    if (!data.code_content && !data.github_repo_url) {
        throw new Error('Debe proporcionar el código fuente o una URL de repositorio para la auditoría.');
    }
    ...
    const newRequest = {
        ...
        code_content: data.code_content || null, // Almacenar el código si se proporcionó directamente
        phase: data.phase,
        week: data.week,
...
export async function getReviewDetails(reviewId) {
    ...
    return {
        ...
        review: review ? {
            ...
            mensaje_tutor: review.mensaje_tutor || null,
...
export async function saveAIReview(reviewRequestId, reviewData, reviewerUserId) {
    ...
    const newReview = {
        ...
        mensaje_tutor: reviewData.mensaje_tutor || null,
